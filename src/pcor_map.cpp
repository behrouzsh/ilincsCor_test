...
void *cor_thread_map_fn(void *cor_thread_data);
...
t_output pcor_map(t_input input, t_input input_weights, t_input_map input_map, t_data_matrix *data_matrix, int number_of_threads) 
{
   ...
   for (int thread_i = 0; thread_i < number_of_threads; thread_i++)
   {
      ...
      cor_thread_params[thread_i].input_map = &input_map;
      cor_thread_params[thread_i].data_matrix = data_matrix;

      pthread_create(&running_thread[thread_i], NULL, cor_thread_map_fn, &cor_thread_params[thread_i]);
   }
...
void *cor_thread_map_fn(void *cor_thread_data) {
   ...
   t_output result = cor_map(*input, *input_weights, *input_map, data_matrix, each_n_cols, each_start_col);
